<?php
	...
	$group_id = $_GET['id'];
		...
		if(isset($_POST['share'])){
			...
			mysqli_query($con,"INSERT INTO groupposts(group_id,member_id,posts,date) VALUE ('$group_id','$member_id','$post',NOW())") or die(mysqli_error());}}
		}
		if(isset($_POST['comment'])){
			$comment = $_POST['textarea'];
			$postid = $_POST['ucantseeme'];
			if ($comment !=""){
			mysqli_query($con,"INSERT INTO grouppostcomments(grouppost_id,member_id,date,comments) VALUE ('$postid','$member_id',NOW(),'$comment')")or die(mysqli_error());}
		}
		if(isset($_POST['uploadfile'])){
			...
			if($name!=""){
				...
				if ($error > 0){
					...
					mysqli_query($con,"INSERT INTO groupposts(group_id,member_id,posts,file,date) VALUE ('$group_id','$member_id','$post','$name',NOW())") or die(mysqli_error());
					}
				}
			}else{
            ...
            <div class="navbar-default sidebar" role="navigation" style="margin-top:;">
                <div class="sidebar-nav navbar-collapse">
                    <ul class="nav" id="side-menu">
                        ...
                        <li>
                            <a href="#"><i class="fa fa-users fa-fw"></i> Group Profile<span class="fa arrow"></span></a>
                            <ul class="nav nav-second-level">
                            <?php 
								$sql=mysqli_query($con,"SELECT * FROM groups WHERE group_id='$group_id'") or die(mysqli_error());
								$row=mysqli_fetch_array($sql);
								echo "<li><center><img src='../image/groups/".$row['photo']."' width='130px' height='230px' alt='Image cannot be dispalyed'></center></li>";
								echo "<li><a href='groups.php?id=".$group_id." style='text-decoration:none;'>".$row['group_name']."</a></li>";
								echo "<li><a href='infogroup.php?id=".$group_id." rel='facebox' style='text-decoration:none;'>Info</a></li>";
								echo "<li><a href='#' style='text-decoration:none;'>Photos of ".$row['group_name']."</a></li>";
							?>
                            </ul>
                            <!-- /.nav-second-level -->
                        </li>
                        <li>
                            <a href="#"><i class="fa fa-user fa-fw"></i> <?php echo $row['group_name'];?>'s Members<span class="fa arrow"></span></a>
                            <ul class="nav nav-second-level">
                                <?php
								$post = mysqli_query($con,"SELECT * FROM groupmembers WHERE group_id = '$group_id' AND status='1' LIMIT 0,5")or die(mysqli_error());
								$num_rows  =mysqli_num_rows($post);
								if ($num_rows != 0 ){
									echo "<span class='pull-right'>".$num_rows." "."members</span>";
									while($row = mysqli_fetch_array($post)){
										...
										echo '<li> <a href=infofriend.php?id='.$membera["member_id"].' rel="facebox" style="text-decoration:none;"><img src="../image/members/'. $membera['photo'].'" height="30" width="30"> '.$membera['firstname'].' '.$membera['lastname'].' </a> </li>';
									}
								}
							?>
                            </ul>
